...
FIXED VERSION:
- Regime Detection verwendet SHIFT(1) um Look-Ahead Bias zu vermeiden
- Alle Indikatoren basieren nur auf vergangenen Daten
"""
...
print("=" * 70)
print("FEATURE ENGINEERING (EXP 4: MARKET REGIME + ENSEMBLE) - FIXED")
print("=" * 70)
...
# 7. --- FIXED: MARKET REGIME DETECTION (NO LOOK-AHEAD BIAS) ---
print("   ✅ Detecting Market Regimes (FIXED - No Look-Ahead Bias)...")
...
# FIX: Shift all indicators by 1 to avoid look-ahead bias
# This ensures we only use information available BEFORE the current timestamp
df['ma_20_lag'] = df['ma_20'].shift(1)
df['ma_50_lag'] = df['ma_50'].shift(1)
df['ma_200_lag'] = df['ma_200'].shift(1)

# Trend Slope (20-day MA slope) - shifted
df['ma_slope'] = df['ma_20_lag'].pct_change(20)

# Volatility (20-day rolling std of returns) - shifted
df['volatility'] = df['log_return'].shift(1).rolling(20).std()

# Volume Trend - shifted
df['volume_ma'] = df['volume'].shift(1).rolling(20).mean()
df['volume_ratio'] = df['volume'].shift(1) / df['volume_ma']

# Regime Classification (using lagged indicators)
...
conditions = [
    (df['ma_slope'] > 0.02) & (df['volatility'] < vol_33) & (df['close'].shift(1) > df['ma_200_lag']),  # Bull
    (df['ma_slope'] < -0.02) & (df['volatility'] > vol_67) & (df['close'].shift(1) < df['ma_200_lag']), # Bear
]
...
print(f"\n✅ Feature Engineering abgeschlossen (FIXED VERSION).")
